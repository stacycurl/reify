...
case class Fields(fields: List[Field]) {
  ...
  private def reifyField(owner: Any, field: java.lang.reflect.Field): Either[String, Reified] = {
    field.setAccessible(true)

    for {
      fieldValue <- Try(field.get(owner)).toEither.left.map(_.getMessage)
      reify <- Reify.find(fieldValue).toRight("Could not find reify instance")
...
object Fields {
  def create(value: Any): Fields =
    Fields(value.getClass.getDeclaredFields.toList)
}